  ...
  implements IEventHandler<ConsumeRootEvent>
{
  ...
  handle(event: ConsumeRootEvent) {
    ...
    // console.log('produceMessage', event)
    console.log('produceMessage', produceMessage)
    this.eventProducerService.publisher('dsb.createOrganization', produceMessage);
    // this.eventProducerService.publisher('dsb.createOrganization', event.body);
  }
}
